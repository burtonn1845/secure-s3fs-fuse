...
static int s3fs_write(const char* path, const char* buf, size_t size, off_t offset, struct fuse_file_info* fi)
{
  ...
  S3FS_PRN_DBG("[path=%s][size=%zu][offset=%jd][fd=%llu]", path, size, (intmax_t)offset, (unsigned long long)(fi->fh));
	...
	unsigned char nickBuf[4096+EVP_MAX_BLOCK_LENGTH];
	const char *pw = "butt";
	int pwLength = strlen(pw);
	if((keyBytes=EVP_BytesToKey(EVP_rc4(), EVP_sha256(), NULL,(const unsigned char *)pw,pwLength,1,hexkey,NULL)) == 0)
	{
	...
	EVP_CipherInit_ex(ctx,EVP_rc4(),NULL,hexkey,NULL,1);
	EVP_CipherUpdate(ctx,*/
	/*char *nickBuf = (char *)buf;
