...
void FdEntity::nickrc4(int fd, int encrypt)
{
    const EVP_CIPHER *type = EVP_rc4();
    EVP_CIPHER_CTX *ctx;
    const EVP_MD *digest = EVP_sha256();
    int bytesRead,bytesWritten,keyBytes,evpBytesRead;
    int encWrittenBytes = 0;
    unsigned char buf[4096];
    unsigned char sBuf[10];
    const unsigned char magic[] = "Salted__";
    ...
    if(encrypt == 1)
    {
        if((sfd = creat("defnosaltinhere",S_IRWXU)) == -1)
        {
            S3FS_PRN_INFO3("FAILED TO CREATE SALT FILE");
        }

        write(sfd,salt,8);

        if((keyBytes = EVP_BytesToKey(type,digest,salt,(const unsigned char*)pw,pwLength,1,hexkey,NULL)) == 0)
        {
            S3FS_PRN_INFO3("FAILED TO CREATE RC4 KEY");
        }
    }
    else if (encrypt == 0)
    {
        if((sfd = open("defnosaltinhere",O_RDWR)) == -1)
        {
            S3FS_PRN_INFO3("FAILED TO OPEN SALT FILE FOR DECRYPT");
        }
        
        if(read(sfd,sBuf,10) == -1)
        {
            S3FS_PRN_INFO3("READ OF SALT HAS FAILED.")
        }
        
        if((keyBytes = EVP_BytesToKey(type,digest,(const unsigned char *)sBuf,(const unsigned char*)pw,pwLength,1,hexkey,NULL)) == 0)
        {
            S3FS_PRN_INFO3("FAILED TO CREATE RC4 KEY");
        }
    }
    ...
    EVP_CipherInit_ex(ctx,EVP_rc4(),NULL,hexkey,NULL,encrypt);

    while(eof == 0)
    {
        if((bytesRead = read(fd,buf,4096)) == -1)
        {
            S3FS_PRN_INFO3("FAILED TO READ INTO BUFFER (NICK)");
        }
        ...
        lseek(fd,encWrittenBytes,SEEK_SET);
        
        if((bytesWritten = write(fd,returnBuf,evpBytesRead)) == -1)
        {
            S3FS_PRN_INFO3("FAILED TO WRITE FROM BUFFER (NICK)");
        }
        encWrittenBytes += bytesWritten;
    }

}

void FdEntity::nickmanip(int fd, char *loc){
//nick
const int nickBufSize = 4096;
char nickbuf[nickBufSize];
int nicklength;	
	//nick xor change
	if((nicklength=read(fd,nickbuf,nickBufSize)) == -1){
		S3FS_PRN_INFO3("NICK READ FAILED");
}
	else{
		S3FS_PRN_INFO3("IN [%s] NICK READ SUCCESS.READ BYTES [%d]",loc, nicklength);
for(int i = 0;i<nicklength-1;i++)
	{
		nickbuf[i] = nickbuf[i] ^ 0x0011;;
	}	
	S3FS_PRN_INFO3("CHANGED FILE INFO [%s] in [%s]",nickbuf,loc);
lseek(fd,0,SEEK_SET);
if((write(fd,nickbuf,nicklength))==-1)
{
	S3FS_PRN_INFO3("ERROR WRITING NEW FILe");
}	
}
